package com.xiongyi.controller.business.order;
...
import com.xiongyi.service.business.order.OrderManager;
...
public class OrderController extends BaseController {
	...
	public ModelAndView save() throws Exception{
		...
		pd.put("ORDERNO", "");	//订单编号
		pd.put("ORDERSTATUS", "0");	//订单状态
		pd.put("ORDERCREATETIME", "");	//订单创建时间
		pd.put("USERID", "");	//与用户ID关联
		pd.put("ORDERCANCELTIME", "");	//订单取消时间
		pd.put("DISCUSSSID", "");	//关联评论ID
	...
	public ModelAndView exportExcel() throws Exception{
		...
		for(int i=0;i<varOList.size();i++){
			...
			vpd.put("var3", varOList.get(i).getString("ORDERSTATUS"));	//3
			vpd.put("var4", varOList.get(i).getString("ORDERCREATETIME"));	//4
			vpd.put("var5", varOList.get(i).getString("USERID"));	//5
			vpd.put("var6", varOList.get(i).getString("ORDERCANCELTIME"));	//6
			vpd.put("var7", varOList.get(i).getString("DISCUSSSID"));	//7
			varList.add(vpd);
		}
